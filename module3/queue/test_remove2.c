 ...
 * Version: 1.0
 * 
 * Description: remove a node not in queue
 * 
 */
...
int main() {
    ...
    // Remove
    car_t *result = (car_t*)qremove(qp_1, (bool (*)(void*, const void*))compareCar, "Tesla");

    if (result == NULL && qget(qp_1) == car_p && qget(qp_1) == car2_p && qget(qp_1) == car3_p && qget(qp_1) == car4_p && qget(qp_1) == car5_p ) {
        exit(EXIT_SUCCESS);
    } else {
        exit(EXIT_FAILURE);
    }
