 ...
 * Version: 1.0
 * 
 * Description: Test for invalid keylen on hput
 * 
 */
...
int main() {
    ...
    // Test for invalid keylen
    if (hput(htp, person1, person1->studentID, -3) != 0 && hput(htp, person2, person2->studentID, 6) == 0 && hput(htp, person3, person3->studentID, 6) == 0) {
        exit(EXIT_SUCCESS);
    } else {
        exit(EXIT_FAILURE);
    }
